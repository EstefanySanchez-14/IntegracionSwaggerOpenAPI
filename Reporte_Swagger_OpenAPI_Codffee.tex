...
\capturaPendiente{Compilación exitosa del proyecto}{Insertar captura de la terminal con el comando \texttt{mvn -DskipTests package} y el mensaje \texttt{BUILD SUCCESS}.}
...
\capturaPendiente{Interfaz Swagger UI cargada}{Insertar captura de Swagger UI mostrando los módulos Autenticación, Productos, Categorías, Pedidos, Usuarios, Perfil, Reportes, Correos y Prueba.}
...
\capturaPendiente{Especificación OpenAPI JSON}{Insertar captura del navegador mostrando el documento JSON generado en \texttt{/v3/api-docs}.}
...
\capturaPendiente{Endpoint de autenticación documentado}{Insertar captura de \texttt{POST /api/auth/login} expandido, mostrando cuerpo de solicitud y respuestas posibles.}

\capturaPendiente{Autorización con JWT en Swagger UI}{Insertar captura del botón \textit{Authorize} o de un endpoint protegido mostrando el candado de seguridad.}

\capturaPendiente{Prueba de endpoint público}{Insertar captura de \textit{Try it out} en \texttt{GET /api/prueba} o \texttt{GET /api/productos/disponibles} con respuesta HTTP 200.}

\capturaPendiente{Documentación de pedidos y reportes}{Insertar captura de los endpoints de pedidos y reportes PDF dentro de Swagger UI.}
...
\section{Lista de capturas pendientes}

\begin{enumerate}
    \item Terminal con \cod{mvn -DskipTests package} y resultado \cod{BUILD SUCCESS}.
    \item Docker Desktop iniciado o backend ejecutándose localmente.
    \item Swagger UI en \url{http://localhost:8080/swagger-ui/index.html}.
    \item Especificación OpenAPI JSON en \url{http://localhost:8080/v3/api-docs}.
    \item Endpoint \cod{POST /api/auth/login} expandido.
    \item Botón \textit{Authorize} con token Bearer JWT.
    \item \cod{GET /api/productos/disponibles} probado desde Swagger UI.
    \item \cod{GET /api/categorias/activas} probado desde Swagger UI.
    \item Endpoints de pedidos documentados.
    \item Endpoint de reportes PDF mostrando respuesta \cod{application/pdf}.
\end{enumerate}
...
Finalmente, el proyecto quedó preparado para documentar los módulos principales solicitados: productos, categorías, pedidos, autenticación, seguridad y reportes. La compilación del backend fue exitosa, quedando pendiente únicamente levantar MySQL o Docker Desktop para tomar las capturas finales de funcionamiento.
